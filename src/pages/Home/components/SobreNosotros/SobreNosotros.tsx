import '../../styles.css'

export const SobreNosotros = () => {
    return (
        <>
            <h1 className="title-about-us-section" id="about-us">SOBRE NOSOTROS</h1>
            <div className='body-about-us'>
                <p>Somos el taller del departamento de electrónica del Industrial de Temperley. Es un espacio para desarrollar proyectos de electrónica vinculando el contenido de las materias con las nuevas tecnologias como robótica, inteligencia artificial, cloud computing, software, etc. Te invitamos a explorar nuestra página y conocer más sobre los proyectos. ¡Gracias por visitar nuestra página web!</p>
            </div>
        </>
    )
}
